...
| **数据访问层** | 数据持久化抽象、Repository 模式 | `store.py` |
| **基础设施层** | 数据库、HTTP 客户端、日志、配置 | `config.py`, SQLite |
...
├── store.py                # 数据存储 - TradingStore + SqliteTradingStore
│
├── api/                    # API 路由层
│   ├── __init__.py
│   ├── deps.py             # 依赖注入
...
class TradingStore(ABC):
    ...
    @abstractmethod
    def get_positions(self, status: str | None = None) -> list[PositionRecord]: ...
```

**目的**：
- 解耦业务逻辑与具体数据库实现
- 便于单元测试（可 Mock TradingStore）
- 支持未来更换数据库（如 PostgreSQL）
...
```python
def get_db() -> SqliteTradingStore: ...
def get_exchange(db: SqliteTradingStore = Depends(get_db)) -> MockExchange: ...
